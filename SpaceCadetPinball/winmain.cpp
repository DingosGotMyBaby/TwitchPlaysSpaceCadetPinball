...
int winmain::WinMain(LPCSTR lpCmdLine) {
...
#ifndef __EMSCRIPTEN__ // ifndef means if not defined
    BasePath = SDL_GetBasePath();
#else
    BasePath = strdup("/game_resources/");
    chdir(BasePath);
#endif
    // Default volume is 0.5f (max).
    float gameVolume = 0.5f;
    const char *volArg = strstr(lpCmdLine, "-vol");
    if (volArg) {
        // Expecting a value after "-vol", e.g. "-vol 0.5"
        gameVolume = strtof(volArg + 4, nullptr);
    }
    // clamp gameVolume to min and max values
    ...
    // PB init from message handler
    {
        ...
        if (sdlGameVolume > SDL_MIX_MAXVOLUME) {
            sdlGameVolume = SDL_MIX_MAXVOLUME;
        }
        ...
        // std::cout << "Volume is " << Mix_MasterVolume(-1) << "and was " << globalVolBefore << std::endl;
        // std::cout << "Volume is " << Mix_Volume(-1, -1) << "and was " << allChannelsVolBefore << std::endl;



        if (gameVolume > 0.0f && !pinball::quickFlag && !midi::music_init(gameVolume))
...
/**
 * This function is a helper function to trigger in game events from Javascript
...
// Emscripten bindings for the web interface
#ifdef __EMSCRIPTEN__
EMSCRIPTEN_BINDINGS(winmain) {
    emscripten::function("TriggerEvent", &winmain::TriggerEvent);
}
#endif
